...
make_dragons(int stop_before_owl)
{
...

  {
	...
	TRACE_T("eye at %1m found for dragon at %1m--augmenting genus\n",
	      str, dr);
...
find_lunches()
{
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++)
	...
	if (lunch == NO_MOVE
	  ...
	  TRACE_T("at %1m setting %1m.lunch to %1m (cutstone=%d)\n",
		str, origin,
...
{ 
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++) {
	...
	&& black_eye[str].origin == str) {
      ...
      DEBUG_T(DEBUG_EYES, "Black eyespace at %1m: %s\n", str,
	...
	&& white_eye[str].origin == str) {
      ...
      DEBUG_T(DEBUG_EYES, "White eyespace at %1m: %s\n", str,
	    eyevalue_to_string(&value));
...
revise_inessentiality()
{
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++)
	...
	if (!essential && neighbors > 0) {
	  DEBUG_T(DEBUG_WORMS, "Worm %1m revised to be inessential.\n", str);
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++) {
	...
	if (!worm[w].inessential)
      ...
      if (w == NO_MOVE) {
	DEBUG_T(DEBUG_DRAGONS, "Dragon %1m revised to be inessential.\n", str);
...
identify_thrashing_dragons()
{
  ...
  DEBUG_T(DEBUG_DRAGONS, "thrashing dragon found at %1m\n", thrashing_dragon);
...
set_strength_data(int color, char safe_stones[BOARDMAX],
    		  float strength[BOARDMAX])
{
  gg_assert(IS_STONE(color) || color == EMPTY_T);
...
compute_dragon_genus(int d, struct eyevalue *genus, int eye_to_exclude)
{
  ...
  if (board[d] == BLACK) {
	...
	continue;
	  ...
	  && is_same_dragon(dr, d)) {
	TRACE_T("eye at %1m (%s) found for dragon at %1m--augmenting genus\n",
...
join_dragons(int d1, int d2)
{
  ...
  if (worm[d1].size > worm[d2].size
	  ...
	  && d1 < d2)) {
    origin = d1;
    DEBUG_T(DEBUG_DRAGONS, "joining dragon at %1m to dragon at %1m\n", d2, d1);
  }
  else {
    origin = d2;
    DEBUG_T(DEBUG_DRAGONS, "joining dragon at %1m to dragon at %1m\n", d1, d2);
...
compute_dragon_weakness_value(int d)
{
  ...
  DEBUG_T(DEBUG_DRAGONS, "Computing weakness of dragon at %1m:\n", origin);
  ...
  DEBUG_T(DEBUG_DRAGONS, " result: %f.\n", weakness);
  return weakness;
}
