...
{
  owl_analyze_semeai_after_move(PASS_MOVE, EMPTY_T, apos, bpos, resulta, resultb,
			      ...
			      int recompute_dragons)
{
  ...
  count_variations = 1;
  if (move == PASS_MOVE)
    TRACE_T("owl_analyze_semeai: %1m vs. %1m\n", apos, bpos);
  else
    TRACE_T("owl_analyze_semeai_after_move %C %1m: %1m vs. %1m\n",
		      ...
		      int *move, int pass, int owl_phase)
{
  ...
  if (stackp <= semeai_branch_depth && (hashflags & HASH_SEMEAI)
    ...
    *resultb = value2;
    ...
    TRACE_T("%oVariation %d: %1m %1m %s %s %1m (cached) ",
  ...
  for (k = 0; k < MAX_SEMEAI_MOVES; k++) {
	...
	*resultb = WIN;
	*move = mpos;
	TRACE_T("After %1m I (%C) am alive, you are dead\n", mpos, color);
			   ...
			   int *this_resulta, int *this_resultb)
{
  ...
  *this_resultb = 0;
  ...
  TRACE_T("Trying %C %1m. Current stack: ", color, move);
  if (verbose) {
    dump_stack();
    goaldump(owla->goal);
    gprintf("\n");
  ...
  if (count_variations >= semeai_node_limit) {
    TRACE_T("Out of nodes, claiming win.\n");
    result_certain = 0;
    *this_resulta = WIN;
    *this_resultb = WIN;
  }
		  ...
		  int raw_value, int *critical_semeai_worms)
{
  ...
  ASSERT1(board[move] == EMPTY_T, move);
  verbose = 0;
...
second_liberty_of_goal(int pos, struct local_owl_data *owl)
{
  int k;
  for (k = 0; k < 4; k++)
    if (board[pos + delta[k]] == EMPTY_T && liberty_of_goal(pos + delta[k], owl))
      return 1;
...
reading_limit_reached(const char **live_reason, int this_variation_number)
{
  ...
  if (stackp > owl_reading_depth) {
    TRACE_T("%oVariation %d: ALIVE (maximum reading depth reached)\n",
  ...
  if (local_owl_node_counter >= owl_node_limit) {
    result_certain = 0;
    TRACE_T("%oVariation %d: ALIVE (owl node limit reached)\n",
	  this_variation_number);
	      ...
	      struct local_owl_data *owl, int escape)
{
  ...
  if ((hashflags & HASH_OWL_ATTACK)
	...
	*move = xpos;
    ...
    if (value1 == WIN)
      TRACE_T("%oVariation %d: DEAD (cached)\n", this_variation_number);
    else
      TRACE_T("%oVariation %d: ALIVE (cached)\n", this_variation_number);
  ...
  if (owl_estimate_life(owl, NULL, vital_moves, &live_reason, 1,
	...
	if (owl_goal_worm[k] == NO_MOVE)
	  break;
	if (board[owl_goal_worm[k]] == EMPTY_T
  ...
  for (pass = 0; pass < 6; pass++) {
	...
	/* No defense, we won. */
	if (dcode == 0) {
	  TRACE_T("%oVariation %d: DEAD (no defense)\n",
		this_variation_number);
	...
	continue;
      ...
      TRACE_T("Trying %C %1m. Escape = %d. Current stack: ",
	    other, mpos, escape);
      if (verbose)
	dump_stack();
	 ...
	 */
	  ...
	  for (l = 0; l < MAX_GOAL_WORMS; l++) {
	    if (owl_goal_worm[l] == NO_MOVE)
	      break;
	    if (board[owl_goal_worm[l]] == EMPTY_T)
...
owl_threaten_attack(int target, int *attack1, int *attack2)
{
  ...
  gg_assert(stackp == 0);
  TRACE_T("owl_threaten_attack %1m\n", target);
  init_owl(&owl, target, NO_MOVE, NO_MOVE, 1, NULL);
	        ...
	        int escape)
{
  ...
  if (stackp > 2 && escape_route >= 5) {
     ...
     * escaping.
     */
    TRACE_T("%oVariation %d: ALIVE (escaped)\n", this_variation_number);
  ...
  for (pass = 0; pass < 4; pass++) {
	...
	if (goalcount < 5) {
	  ...
	  if (attack_and_defend(str, NULL, NULL, NULL, &dpos)
	    ...
	    TRACE_T("Found tactical defense for %1m at %1m.\n", str, dpos);
...
owl_threaten_defense(int target, int *defend1, int *defend2)
{
  ...
  TRACE_T("owl_threaten_defense %1m\n", target);
  init_owl(&owl, target, NO_MOVE, NO_MOVE, 1, NULL);
  ...
  *eyemax = 0;
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (does_attack && attack_point != NO_MOVE) {
	  ...
	  TRACE_T("%s at %1m, score %d (eye at %1m, value %s, pessimistic_min %d)\n",
	  ...
	  if (eye[attack_point].marginal
	    ...
	    TRACE_T("vital point looked stupid, moved it to %1m\n",
		  attack_point);
   ...
   */
  {
	...
	*eyemax += lunch_max;
	...
	if (does_attack) {
	  ...
	  TRACE_T("save lunch at %1m with %1m, score %d, probable eye %d, max eye %d\n",
	  ...
	  TRACE_T("eat lunch at %1m with %1m, score %d, probable eye %d, max eye %d\n",
			      ...
			      int is_attack_point)
{
  ...
  for (k = 0; k < 4; k++) {
    ...
    if (board[*vital_point + up] != EMPTY_T)
      continue;

    right = delta[(k+1) % 4];

    if (board[*vital_point + right] != EMPTY_T
	|| board[*vital_point - right] != EMPTY_T)
      continue;

    if (board[*vital_point + 2 * up] != EMPTY_T
	|| board[*vital_point + up + right] != EMPTY_T
	|| board[*vital_point + up - right] != EMPTY_T) {
      *vital_point += up;
      return 1;
    }
...
static int
check_pattern_hard(int move, int color, struct pattern_b *pattern, int ll)
{
   ...
   */
  if (!(pattern->class_T & CLASS_s) && !safe_move_checked) {
    if (!owl_safe_move(move, color)) {
      if (0)
	TRACE_T("  move at %1m wasn't safe, discarded\n", move);
      return 0;
    }
    if (!is_legal(move, color)) {
      if (0)
	TRACE_T("  move at %1m wasn't legal, discarded\n", move);
      return 0;
    }
  }
   ...
   */
  if (pattern->class_T & CLASS_n) {
    if (safe_move(move, OTHER_COLOR(color)) == 0) {
      if (0)
	TRACE_T("  opponent can't play safely at %1m, move discarded\n", move);
      return 0;
    }
  }
...
close_pattern_list(int color, struct matched_patterns_list_data *list)
{
  if (list->initialized) {
	...
	if (check_pattern_hard(list->pattern_heap[i]->move, color,
	  ...
	  if (!found_one) {
	    TRACE_T("Remaining valid (but unused) patterns at stack: ");
	    dump_stack();
	    found_one = 1;
	  }
      	  TRACE_T("Pattern %s found at %1m with value %d\n",
 ...
 */
static void
collect_owl_shapes_callbacks(int anchor, int color, struct pattern_b *pattern,
                             int ll, void *data)
{
  struct matched_patterns_list_data *matched_patterns = data;
  struct matched_pattern_data *next_pattern;

  UNUSED_T(color); /* The calling function has to remember that. */
  ...
  do {
	...
	case REVERSE_SENTE:
	  if (num_reverse_sente < MAX_STORED_REASONS) {
	    ASSERT1(board[target] == EMPTY_T, target);
...
pattern_list_prepare(struct matched_patterns_list_data *list)
{
  ...
  for (k = 0; k < list->counter; k++) {
     ...
     * patterns in chains.
     */
    if (!(list->pattern_list[k].pattern->class_T & CLASS_c))
			...
			struct owl_move_data *moves, int cutoff)
{
  ...
  while (list->heap_num_patterns > 0) {
	...
	continue;
      ...
      if (pattern && !(pattern->class_T & CLASS_c)) {
	...
	if (pattern->class_T & CLASS_B)
	...
	else if (pattern->class_T & CLASS_b) {
	  int i;
	  int same_dragon = 1;
	  ...
	  for (i = 0; i < list->heap_num_patterns; i++) {
		...
		&& ((pattern_data->pattern->class_T & CLASS_B)
		    || !(pattern_data->pattern->class_T & CLASS_b))) {
	      if (check_pattern_hard(move, color, pattern_data->pattern,
				     pattern_data->ll)) {
		TRACE_T("Additionally pattern %s found at %1m\n",
		      pattern_data->pattern->name, move);
		if (pattern_data->pattern->class_T & CLASS_B)
		  same_dragon = 0;
		else
		  same_dragon = 2;
	 ...
	 */
	if (pattern && pattern->class_T & CLASS_B)
	  moves[k].same_dragon = 0;
	else
	  moves[k].same_dragon = 2;
      }

      if (pattern && pattern->class_T & CLASS_E)
	moves[k].escape = 1;
      else
	moves[k].escape = 0;
 ...
 */
static void
owl_shapes_callback(int anchor, int color, struct pattern_b *pattern,
		    int ll, void *data)
{
  ...
  if (pattern->helper) {
    ...
    gg_assert(0);
    DEBUG_T(DEBUG_HELPER, "  asking helper to consider '%s'+%d at %1m\n",
	  pattern->name, ll, move);
    tval = pattern->helper(pattern, ll, move, color);
    
    if (tval > 0) {
      DEBUG_T(DEBUG_HELPER, "helper likes pattern '%s' value %d at %1m\n",
	    pattern->name, tval, move);
    }
    else {
      DEBUG_T(DEBUG_HELPER, "  helper does not like pattern '%s' at %1m\n",
	    pattern->name, move);
...
owl_mark_boundary(struct local_owl_data *owl)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (board[pos2] == color
	    && !owl->neighbors[pos2]
	    && (board[SOUTH(gg_min(pos, pos2))] == EMPTY_T
		|| board[NORTH(gg_max(pos, pos2))] == EMPTY_T))
    		...
    		struct local_owl_data *owl, int semeai_call)
{
   ...
   * goal.
   */
  if (!semeai_call && lunch != NO_MOVE && board[lunch] != EMPTY_T) {
...
test_owl_attack_move(int pos, int dr, int kworm, int acode)
{
  ...
  if (DRAGON2(dr).semeais == 0
    ...
    DEBUG_T(DEBUG_OWL, "owl: %1m attacks %1m (%s) at move %d\n",
	...
	&& (semeai_result >= REVERSE_RESULT(acode))) {
      ...
      DEBUG_T(DEBUG_OWL, "owl: %1m ineffective owl attack on %1m (can live in semeai with %1m)\n", pos, dr, dr2);
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (DRAGON2(pos).owl_defense_point != NO_MOVE) {
	  if (DRAGON2(pos).owl_defense_code == LOSS) {
	    ...
	    DEBUG_T(DEBUG_OWL, "owl: %1m defends %1m with loss at move %d\n",
	    ...
	    DEBUG_T(DEBUG_OWL, "owl: %1m defends %1m at move %d\n",
		  DRAGON2(pos).owl_defense_point, pos, movenum+1);
	  }
	}
      }
	...
	DEBUG_T(DEBUG_OWL, "owl: %1m threatens to defend %1m at move %d\n", 
	      DRAGON2(pos).owl_defense_point, pos, movenum+1);
      }
      if (board[pos] == color
	...
	DEBUG_T(DEBUG_OWL, "owl: %1m prevents a threat against %1m at move %d\n",
	...
	if (DRAGON2(pos).owl_attack_point != NO_MOVE) {
	  ...
	  DEBUG_T(DEBUG_OWL, "owl: %1m threatens %1m at move %d\n",
		DRAGON2(pos).owl_attack_point, pos, movenum+1);
	}
	...
	DEBUG_T(DEBUG_OWL, "owl: %1m attacks %1m with gain at move %d\n", 
	      DRAGON2(pos).owl_attack_point, pos, movenum+1);
      }
	...
	add_owl_defense_move(DRAGON2(pos).owl_defense_point, pos,
			     DRAGON2(pos).owl_defense_code);
	DEBUG_T(DEBUG_OWL, "owl: %1m defends %1m against possible loss at move %d\n",
	      DRAGON2(pos).owl_defense_point, pos, movenum+1);
...
owl_does_defend(int move, int target, int *kworm)
{
  ...
  TRACE_T("owl_does_defend %1m %1m(%1m)\n", move, target, origin);
...
owl_confirm_safety(int move, int target, int *defense_point, int *kworm)
{
  ...
  origin = dragon[target].origin;
  TRACE_T("owl_confirm_safety %1m %1m(%1m)\n", move, target, origin);
...
owl_does_attack(int move, int target, int *kworm)
{
  ...
  origin = dragon[target].origin;
  TRACE_T("owl_does_attack %1m %1m(%1m)\n", move, target, origin);
...
owl_connection_defends(int move, int target1, int target2)
{
  ...
  TRACE_T("owl_connection_defends %1m %1m %1m\n", move, target1, target2);
...
owl_find_lunches(struct local_owl_data *owl)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (acode != 0) {
	  ...
	  ASSERT1(board[apos == EMPTY_T], lunch);
	  if (dcode != 0) {
	  ...
	  if (!essential) {
	    TRACE_T("Inessential string found at %1m.\n", lunch);
	    for (r = 0; r < num_stones; r++)
...
improve_lunch_attack(int lunch, int attack_point)
{
  ...
  if (safe_move(attack_point, color)) {
	...
	for (k = 0; k < 4; k++) {
	  ...
	  if (!ON_BOARD(attack_point - delta[k]) && board[apos] == EMPTY_T) {
  ...
  for (k = 0; k < 4; k++) {
	...
	&& does_attack(defense_point, lunch)) {
      TRACE_T("Moved attack of lunch %1m from %1m to %1m.\n",
...
improve_lunch_defense(int lunch, int defense_point)
{
  ...
  for (k = 0; k < 4; k++) {
	...
	TRACE_T("Moved defense of lunch %1m from %1m to %1m.\n",
...
compute_owl_escape_values(struct local_owl_data *owl)
{
  ...
  DEBUG_T(DEBUG_ESCAPE, "Owl escape values:\n");
  for (m = 0; m < board_size; m++) {
	...
	if (dragon[pos].crude_status == ALIVE)
      ...
      DEBUG_T(DEBUG_ESCAPE, "%o%d", owl->escape_values[pos]);
    }
    DEBUG_T(DEBUG_ESCAPE, "%o\n");
  }
}
