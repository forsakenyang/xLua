		  ...
		  int do_capture_dead_stones)
{
  ...
  if (best_scoring_move != NO_MOVE
    ...
    *aftermath_move = best_scoring_move;
    DEBUG_T(DEBUG_AFTERMATH, "Closing edge at %1m\n", best_scoring_move);
    return 1;
  }
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    if (board[pos] != EMPTY_T || distance[pos] != -1)
      continue;
  ...
  while (1) {
	...
	*aftermath_move = move;
	DEBUG_T(DEBUG_AFTERMATH, "Splitting eyespace at %1m\n", move);
	return 1;
      }
    }
  }
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	continue;
      ...
      *aftermath_move = move;
      DEBUG_T(DEBUG_AFTERMATH, "Filling opponent liberty at %1m\n", move);
      return 1;
    }
  }
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	&& !is_illegal_ko_capture(worm[pos].attack_points[0], color)) {
      *aftermath_move = worm[pos].attack_points[0];
      DEBUG_T(DEBUG_AFTERMATH, "Tactically attack %1m at %1m\n",
	    pos, *aftermath_move);
      return 1;
    }
  }
...
reduced_genmove(int *move, int color)
{
  ...
  *move = NO_MOVE;  
  ...
  if (review_move_reasons(move, &val, color, 0.0, our_score, NULL))
    TRACE_T("Move generation likes %1m with value %f\n", *move, val);
  gg_assert(stackp == 0);
  ...
  if (val < 0.0) {
    ...
    *move = NO_MOVE;
    ...
    TRACE_T("reduced_genmove() recommends %1m with value %f\n", *move, val);
...
do_play_aftermath(int color, struct aftermath_data *a)
{
  ...
  DEBUG_T(DEBUG_AFTERMATH, "The aftermath starts.\n");
  ...
  while (pass < 2 && moves < board_size * board_size) {
    ...
    moves++;
    DEBUG_T(DEBUG_AFTERMATH, "%d %C move %1m (nodes %d, %d  total %d, %d)\n",
...
play_aftermath(int color)
{
  ...
  static int current_color = EMPTY_T;
  int cached_board = 1;
