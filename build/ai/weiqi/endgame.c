...
endgame_analyze_worm_liberties(int pos, int color)
{
   ...
   * worm have appeared.
   */
  if (k == inessential_liberties && board[pos] != EMPTY_T) {
  ...
  if (k == inessential_liberties && board[pos] != EMPTY_T) {
	...
	if (trymove(attacks[k], other, "endgame", pos)) {
	  if (attack(pos, NULL) != 0) {
	    TRACE_T("  endgame move with territorial value %d.0 found at %1m\n",
		  1, attacks[k]);
...
endgame_find_backfilling_dame(int str, int color_to_move)
{
  ...
  if (move != NO_MOVE && safe_move(move, color)) {
    TRACE_T("  backfilling dame found at %1m for string %1m\n", move, str);
