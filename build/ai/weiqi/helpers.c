...
#define ARGS struct pattern_b *pattern, int trans, int move, int color
...
jump_out_helper(ARGS)
{
  ...
  UNUSED_T(trans); UNUSED_T(pattern);
...
{
  UNUSED_T(trans); UNUSED_T(pattern); UNUSED_T(move);
...
backfill_helper(int apos, int bpos, int cpos)
{
  ...
  if (TRYMOVE(apos, color)) {
	...
	TRACE_T("%o...setting min move value of %1m to 0.1\n", dpos);
      }
      popgo();
    }
    popgo();
...
break_mirror_helper(int str, int color)
{
  if (board_size % 2 == 1
      ...
      && test_symmetry_after_move(PASS_MOVE, EMPTY_T, 1))
    return 1;
