  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
	...
	for (k = 0; k < num_a_threatened_groups - 1; ++k)
	  for (l = k + 1; l < num_a_threatened_groups; ++l) {
	     ...
	     */
	    if (board[a_threatened_groups[k]] == EMPTY_T
		|| board[a_threatened_groups[l]] == EMPTY_T) {
	      if (!attack(ii, NULL)) {
		TRACE_T("Double threat at %1m, either %1m or %1m attacked.\n",
	       ...
	       int save_verbose, int minsize, char goal[BOARDMAX])
{
  ...
  for (k = 0; attacks[k].move != NO_MOVE; k++) {
	...
	*defense_point = apos;
    ...
    DEBUG_T(DEBUG_ATARI_ATARI, "%oreturn value:%d (%1m)\n", aa_val, str);
    return aa_val;
  }
		      ...
		      int last_friendly, int save_verbose, int minsize)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    if (board[last_friendly] != EMPTY_T
			    ...
			    struct pattern_b *pattern, int ll, void *data)
{
  ...
  for (k = 0; k < pattern->patlen; ++k) { /* match each point */
	...
	continue;
       ...
       */
      if (!(pattern->class_T & CLASS_s) && is_self_atari(move, color)) {
	if (countlib(str) > 2)
	  continue;
...
compute_aa_values(int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    aa_values[pos] = value;
    if (1)
      DEBUG_T(DEBUG_ATARI_ATARI, "aa_value for %1m = %d\n", pos, value);
  }
}
	       ...
	       int color)
{
  ...
  for (r = 0; r < queue_end; r++) {
    ...
    for (k = 0; k < 4; k++) {
      ...
      if (!ON_BOARD(pos2))
       continue;
      if ((board[pos] != color || pos == apos) && board[pos2] == EMPTY_T) {
