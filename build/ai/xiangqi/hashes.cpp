...
void RecordHash(const PositionStruct &pos, int nFlag, int vl, int nDepth, int mv) {
  ...
  // 1. 对分值做杀棋步数调整；
  //jjchess __ASSERT_BOUND(1 - MATE_VALUE, vl, MATE_VALUE - 1);
  //jjchess __ASSERT(mv == 0 || pos.LegalMove(mv));
  if (vl > WIN_VALUE) {
  ...
  for (i = 0; i < HASH_LAYERS; i ++) {
	...
	nHashDepth = CHESSMAX((hsh.ucAlphaDepth == 0 ? 0 : hsh.ucAlphaDepth + 256),
        (hsh.wmv == 0 ? hsh.ucBetaDepth : hsh.ucBetaDepth + 256));
	//jjchess __ASSERT(nHashDepth < 512);
...
inline bool MoveStable(PositionStruct &pos, int mv) {
  ...
  // 2. 吃子着法是稳定的；
  //jjchess  __ASSERT(pos.LegalMove(mv));
...
void RecordHashQ(const PositionStruct &pos, int vlBeta, int vlAlpha) {
  ...
  //jjchess __ASSERT((vlBeta > -WIN_VALUE && vlBeta < WIN_VALUE) || (vlAlpha > -WIN_VALUE && vlAlpha < WIN_VALUE));
  ...
  if (lphsh->dwZobristLock0 == pos.zobr.dwLock0) {
    ...
    if (lphsh->dwZobristLock1 == pos.zobr.dwLock1) {
      if (vlHashBeta >= vlBeta) {
		  //jjchess  __ASSERT(vlHashBeta > -WIN_VALUE && vlHashBeta < WIN_VALUE);
        return vlHashBeta;
      }
      if (vlHashAlpha <= vlAlpha) {
		  //jjchess  __ASSERT(vlHashAlpha > -WIN_VALUE && vlHashAlpha < WIN_VALUE);
        return vlHashAlpha;
      }
    }
  }
...
bool PopHash(const PositionStruct &pos) {
  ...
  for (i = 0; i < HASH_LAYERS; i ++) {
    ...
    if (HASH_POS_EQUAL(hsh, pos)) {
      printf("pophash");
      if (hsh.wmv != 0) {
		  //jjchess   __ASSERT(pos.LegalMove(hsh.wmv));
