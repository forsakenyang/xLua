...
semeai()
{
  ...
  if (num_dragons > MAX_DRAGONS) {
    TRACE_T("Too many dragons!!! Might disregard some semeais.");
...
find_moves_to_make_seki()
{
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++) {
	...
	int d = dragon[str].id;
	DEBUG_T(DEBUG_SEMEAI, "Move to make seki at %1m (%1m vs %1m)\n",
	...
	DEBUG_T(DEBUG_SEMEAI, "Move to prevent seki at %1m (%1m vs %1m)\n",
...
semeai_move_reasons(int color)
{
  ...
  for (d = 0; d < number_of_dragons; d++)
	...
	DEBUG_T(DEBUG_SEMEAI, "Adding semeai defense move for %1m at %1m\n",
	...
	DEBUG_T(DEBUG_SEMEAI, "Adding semeai attack move for %1m at %1m\n",
    	      ...
    	      enum dragon_status new_safety)
{
  ...
  if (dragon[dr].status != new_status
    ...
    DEBUG_T(DEBUG_SEMEAI, "Changing status of %1m from %s to %s.\n", dr,
  ...
  if (DRAGON2(dr).safety != new_safety
    ...
    DEBUG_T(DEBUG_SEMEAI, "Changing safety of %1m from %s to %s.\n", dr,
