...
int
computer_move(Gameinfo *gameinfo, int *passes,int *i,int *j)
{
  int move_val;
 ...
 */
 ...
 int
do_move(Gameinfo *gameinfo, int *passes, int nX,int nY)
{
  if (!gnugo_is_legal(nX, nY, gameinfo->to_move)) {
    return 0;
  }
  *passes = 0;
  init_sgf(gameinfo);
  gnugo_play_move(nX, nY, gameinfo->to_move);
  sgffile_add_debuginfo(sgftree.lastnode, 0);
  sgftreeAddPlay(&sgftree, gameinfo->to_move, nX, nY);
  sgffile_output(&sgftree);
...
do_pass(Gameinfo *gameinfo, int *passes, int force)
{
  ...
  return computer_move(gameinfo, passes,-1,-1);
}
