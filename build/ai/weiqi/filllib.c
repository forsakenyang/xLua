...
fill_liberty(int *move, int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    DEBUG_T(DEBUG_FILLLIB, "Filllib: Considering move at %1m.\n", pos);
    ...
    if (safe_move(pos, color) == WIN) {
      DEBUG_T(DEBUG_FILLLIB, "Filllib: Tactically safe.\n");
      if (filllib_confirm_safety(pos, color, &defense_point)) {
	/* Safety confirmed. */
	DEBUG_T(DEBUG_FILLLIB, "Filllib: Safety confirmed.\n");
	*move = pos;
	return 1;
      }
	...
	*move = defense_point;
	 ...
	 */
	DEBUG_T(DEBUG_FILLLIB, "Filllib: Safety not confirmed, discarded.\n");
	TRACE_T("Warning: Blunder detected in fill_liberty().\n");
	continue;
      }
    }
	...
	DEBUG_T(DEBUG_FILLLIB, "Filllib: Backfilling move at %1m.\n", *move);
	...
	if (!is_legal(*move, color)) {
	  DEBUG_T(DEBUG_FILLLIB, "Filllib: Was illegal, discarded.\n");
	  *move = NO_MOVE;
	  continue;
	}
	...
	if (does_capture_something(pos, color)) {
	  ...
	  *move = pos;
       ...
       */
      DEBUG_T(DEBUG_FILLLIB, "Filllib: Illegal, looking for back-capture.\n");
      for (k = 0; k < 4; k++) {
	int d = delta[k];
	if (board[pos + d] == other
	    && worm[pos + d].attack_codes[0] == WIN) {
	  *move = worm[pos + d].attack_points[0];
	  DEBUG_T(DEBUG_FILLLIB, "Filllib: Found at %1m.\n", *move);
	  return 1;
	}
      }
  ...
  /* Nothing found. */
  DEBUG_T(DEBUG_FILLLIB, "Filllib: No move found.\n");
  return 0;
}
