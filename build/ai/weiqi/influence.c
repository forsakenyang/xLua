                       ...
                       struct influence_data *q)
{
  if (q->intrusion_counter >= MAX_INTRUSIONS) {
    DEBUG_T(DEBUG_INFLUENCE, "intrusion list exhausted\n");
    return;
  }
...
reset_unblocked_blocks(struct influence_data *q)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++)
	...
	DEBUG_T(DEBUG_INFLUENCE, "  black block removed from %1m\n", pos);
	q->white_permeability[pos] = 1.0;
      }
      if (!q->safe[pos] && q->black_strength[pos] > 0.0
	  && q->black_permeability[pos] != 1.0) {
	DEBUG_T(DEBUG_INFLUENCE, "  white block removed from %1m\n", pos);
	q->black_permeability[pos] = 1.0;
      }
    }
}
...
add_marked_intrusions(struct influence_data *q, int color)
{
  ...
  for (i = 0; i < q->intrusion_counter; i = j) {
    ...
    if (source_pos == NO_MOVE) {
      ...
      DEBUG_T(DEBUG_INFLUENCE, "Adding %s intrusion at %1m, value %f\n",
 ...
 */
static void
influence_callback(int anchor, int color, struct pattern_b *pattern, int ll,
		   void *data)
{
   ...
   */
  if (((pattern->class_T & CLASS_T) && !(q->is_territorial_influence))
      || ((pattern->class_T & CLASS_W) && q->is_territorial_influence))
    return;
  ...
  if (doing_scoring && (pattern->class_T & CLASS_I))
    return;
  ...
  if (pattern->class_T & (CLASS_A | CLASS_D)) {
   ...
   */
  if ((pattern->class_T & (CLASS_D | CLASS_A | CLASS_B | CLASS_E | CLASS_t))
      && !(pattern->class_T & CLASS_s)) {
    for (k = 0; k < pattern->patlen; ++k) { /* match each point */
      if ((pattern->patn[k].att == ATT_O
	   && (pattern->class_T & (CLASS_D | CLASS_B | CLASS_E | CLASS_t)))
	  || (pattern->patn[k].att == ATT_X
	      && (pattern->class_T & (CLASS_A | CLASS_B | CLASS_t)))) {
	/* transform pattern real coordinate */
	...
	else if (!(pattern->class_T & (CLASS_D | CLASS_B | CLASS_t))) {
	  if ((stackp == 0 && worm[ii].attack_codes[0] != 0)
	      || attack(ii, NULL) != 0)
	    return; /* Match failed */
	}
	/* One test left for class B and t. */
	if ((pattern->class_T & (CLASS_B | CLASS_t))
	...
	if ((pattern->class_T & (CLASS_A | CLASS_t))
	    && pattern->patn[k].att == ATT_X) {
  ...
  DEBUG_T(DEBUG_INFLUENCE, "influence pattern '%s'+%d matched at %1m\n",
	pattern->name, ll, anchor);

  /* For t patterns, everything happens in the action. */
  if ((pattern->class_T & CLASS_t)
   ...
   * source at *.
   */
  if (pattern->class_T & CLASS_I) {
    int this_color = EMPTY_T;
    float strength;
    float attenuation;

    if (q->color_to_move == EMPTY_T || (pattern->class_T & CLASS_s))
    ...
    if (cosmic_gnugo) {
     ...
     if (q == &escape_influence && (pattern->class_T & CLASS_e))
   ...
   */
  for (k = 0; k < pattern->patlen; ++k) { /* match each point */
    if (((pattern->class_T & (CLASS_D | CLASS_A))
	 && (pattern->patn[k].att == ATT_comma
	     || (pattern->patn[k].att == ATT_not
	         && q->is_territorial_influence)))
	|| ((pattern->class_T & CLASS_B)
	    && pattern->patn[k].att == ATT_not)) {
      /* transform pattern real coordinate */
      int ii = AFFINE_TRANSFORM(pattern->patn[k].offset, ll, anchor);
      float strength;
	...
	if (pattern->class_T & CLASS_D)
	  ...
	  blocking_color = OTHER_COLOR(color);
	DEBUG_T(DEBUG_INFLUENCE, "  barrier for %s influence at %1m\n",
	...
	DEBUG_T(DEBUG_INFLUENCE, "  intrusion at %1m\n", ii);
      }
 ...
 */
static void
followup_influence_callback(int anchor, int color, struct pattern_b *pattern,
                            int ll, void *data)
{
  int k;
  int t;
  ...
  for (k = 0; k < pattern->patlen; ++k)  /* match each point */
    if (pattern->patn[k].att == ATT_not) {
      ...
      enter_intrusion_source(anchor, ii, pattern->value,
			     TERR_DEFAULT_ATTENUATION, q);
      DEBUG_T(DEBUG_INFLUENCE, "  followup for %1m: intrusion at %1m\n",
            anchor, ii);
    }
}
...
{
  DEBUG_T(DEBUG_INFLUENCE, "  non-territory for %C at %1m\n", color, pos);
		     ...
		     int move, const char *trace_message)
{
  ...
  if (q != &escape_influence)
    ...
    find_influence_patterns(q, EMPTY_T);
...
whose_loose_territory(const struct influence_data *q, int pos)
{
  ...
  if ((owner == EMPTY_T) && (bi > 10.0 * wi && bi > 7.0 && wi < 40.0))
    owner = BLACK;
  if ((owner == EMPTY_T) && (wi > 10.0 * bi && wi > 7.0 && bi < 40.0))
    owner = WHITE;

  if (ON_BOARD(pos) && IS_STONE(board[pos]) && !q->safe[pos]) { 
    if (0) 
      TRACE_T("dead stone : %1m, territory for %s\n", pos,
                  (owner == WHITE ? "White": 
	       ...
	       int type, int segmentation[BOARDMAX])
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
    if (ON_BOARD(ii)) {
      if ((!marked[ii]) && region_owner(q, ii) != EMPTY_T) {
    			 ...
    			 char escape_value[BOARDMAX])
{
  ...
  escape_influence.color_to_move = EMPTY_T;
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
	...
	escape_value[ii] = 2;
      else if (whose_area(&escape_influence, ii) == EMPTY_T) {
	if (goal) {
	  escape_value[ii] = 0;
			  ...
			  int move)
{
...

  {
    ...
    if (move != -1
	&& this_delta != 0.0)
      DEBUG_T(DEBUG_TERRITORY, "  %1m:   - captured stones %f\n",
	    move, this_delta);
    total_delta += this_delta;
  }
  return total_delta;
