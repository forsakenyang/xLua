...
gameinfo_print(Gameinfo *ginfo)
{
  ...
  if (ginfo->computer_player == WHITE)
    ...
    printf("Black\n");
  else if (ginfo->computer_player == EMPTY_T)
    printf("Both (solo)\n");
  else
    printf("Nobody\n");
}
			  ...
			  const char *untilstr, int orientation)
{
  ...
  if (untilstr) {
    if (*untilstr > '0' && *untilstr <= '9') {
      until = atoi(untilstr);
      DEBUG_T(DEBUG_LOADSGF, "Loading until move %d\n", until);
    }
    else {
	...
	--untiln;
      ...
      DEBUG_T(DEBUG_LOADSGF, "Loading until move at %d,%d (%m)\n", 
  ...
  for (tree->lastnode = NULL; sgftreeForward(tree);) {
	...
	rotate(i, j, &i, &j, board_size, orientation);
	if ((ON_BOARD2(i, j) && board[POS(i, j)] == EMPTY_T)
	    || (i == -1 && j == -1)) {
	  gnugo_play_move(i, j, next);
	  next = OTHER_COLOR(next);
	}
