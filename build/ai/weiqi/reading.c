...
break_through(int apos, int bpos, int cpos)
{
  ...
  ASSERT1(board[dpos] == EMPTY_T , dpos);
  ASSERT1(board[epos] == EMPTY_T , epos);

  /* F might already have been captured. (play_break_through_n() can't
   * detect this.
   */
  if (board[Fpos] == EMPTY_T)
    return 0;
...
special_rescue_moves(int str, int lib, struct reading_moves *moves)
{
  ...
  for (k = 0; k < 4; k++) {
	...
	for (r = 0; r < 4; r++) {
	  if (board[lib + d + delta[r]] == EMPTY_T
...
special_rescue6_moves(int str, int libs[3], struct reading_moves *moves)
{
  ...
  for (r = 0; r < liberties; r++) {
	...
	if (board[apos + up] != EMPTY_T)
	  continue;

	if (board[cpos + right] != EMPTY_T)
	  continue;

	if (board[apos + up + up] == EMPTY_T
	    && board[cpos + up] == EMPTY_T
...
edge_clamp_moves(int str, struct reading_moves *moves)
{
  ...
  for (r = 0; r < adj; r++) {
	...
	if (board[dpos] != EMPTY_T || !liberty_of_string(dpos, apos))
	  continue;

	epos = dpos + up;

	if (board[epos] != EMPTY_T || !liberty_of_string(epos, apos))
	  continue;
                   ...
                   struct reading_moves *moves, int to_move)
{
  ...
  for (r = 0; r < liberties; r++) {
	...
	if (board[apos + up] == other	   /* other on top of liberty */
	...
	else if (board[apos + up] == EMPTY_T  /* empty above the liberty */
		 && board[apos - right + up] == other
		 && board[apos + right] == EMPTY_T) { /* empty to the right */
	  ...
	  if (countlib(apos + up - right) == 1)
	    ...
	    /* Add c if empty */
	    if (board[apos + right + up] == EMPTY_T
...
set_larger_goal_worm(int str, char goal[BOARDMAX])
{
  ...
  for (k = 0; k < 4; k++) {
    ...
    else if (stackp < (12+2) && board[pos] == EMPTY_T) {
      int j;
      for (j = 0; j < 4; j++) {
...
do_tactical_pat(int is_attack, int str, int *move)
{
  ...
  if (verbose > 1) {
    TRACE_T("Moves (variation %d): ", count_variations);
    for (k = 0; k < MAX_READING_MOVES; k++) {
      if (!moves[k].pos)
	break;
      TRACE_T("%o%s@%1m ", moves[k].name, moves[k].pos);
    }
    TRACE_T("\n");
  }
...
find_cap_moves(int str, struct reading_moves *moves)
{
  ...
  for (i = 0; i < numlibs - 1; i++) {
    for (j = i + 1; j < numlibs; j++) {
       ...
       */
      if (BOARD(bi, aj) == EMPTY_T)
        ADD_CANDIDATE_MOVE(POS(bi, aj), 10, *moves, "find_cap");
      else if (BOARD(ai, bj) == EMPTY_T)
...
special_attack3_moves(int str, int libs[2], struct reading_moves *moves)
{
  ...
  for (k = 0; k < 2; k++) {
    ...
    if (apos == SOUTH(bpos) || apos == NORTH(bpos)) {
      if (board[WEST(apos)] == EMPTY_T)
	xpos = WEST(apos);
      else if (board[EAST(apos)] == EMPTY_T)
	xpos = EAST(apos);
      else
	continue;
    }
    else if (apos == WEST(bpos) || apos == EAST(bpos)) {
      if (board[SOUTH(apos)] == EMPTY_T)
	xpos = SOUTH(apos);
      else if (board[NORTH(apos)] == EMPTY_T)
	xpos = NORTH(apos);
      else
	continue;
    }
...
edge_closing_backfill_moves(int str, int apos, struct reading_moves *moves)
{
  ...
  for (k = 0; k < 4; k++) {
    ...
    if (board[apos + up] != color)
      return;
    if (board[apos + right] == EMPTY_T
	&& (!ON_BOARD(apos - right) 
	    || board[apos - right] == color))
      ; /* Everything ok so far. */
    else if (board[apos - right] == EMPTY_T
	     && (!ON_BOARD(apos + right) 
		 || board[apos + right] == color)) {
...
edge_block_moves(int str, int apos, struct reading_moves *moves)
{
  ...
  for (k = 0; k < 4; k++) {
	...
	right = -right;
      ...
      if (board[cpos] == color && board[dpos] == other
	  && board[epos] == EMPTY_T && board[fpos] == EMPTY_T) {
	if (countlib(dpos) == 1) {
	  int gpos = epos + right;
	  ...
	  for (edge_scan = epos; ; edge_scan += right) {
	    ...
	    if (board[edge_scan] != EMPTY_T || board[edge_scan + up] != EMPTY_T)
	      break;
	  }
	}
      }
...
draw_reading_shadow()
{
  ...
  for (i = 0; i < board_size; i++) {
    ...
    for (j = 0; j < board_size; j++) {
      if (!shadow[POS(i, j)] && BOARD(i, j) == EMPTY_T)
	c = '.';
