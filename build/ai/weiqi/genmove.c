	   ...
	   int allowed_moves[BOARDMAX])
{
  ...
  *move = NO_MOVE;  
  ...
  if (play_mirror_go
    ...
    TRACE_T("genmove() recommends mirror move at %1m\n", *move);
    return 1.0;
  }
  ...
  if (val < 10.0 && !doing_scoring && !limit_search) {
	...
	endgame_shapes(color);
      if (review_move_reasons(move, &val, color, pure_threat_value, our_score,
			      allowed_moves)) {
	TRACE_T("Upon reconsideration move generation likes %1m with value %f\n",
	      *move, val); 
      }
    }
  ...
  if (val < 0.0 
    ...
    val = 1.0;
    TRACE_T("Filling a liberty at %1m\n", *move);
  ...
  if (val < 0.0) {
    ...
    *move = NO_MOVE;
    ...
    TRACE_T("genmove() recommends %1m with value %f\n", *move, val);
    /* Maybe time to resign...
     */
    if (resign_allowed && val < 10.0 && should_resign(color, our_score)) {
      TRACE_T("... though, genmove() thinks the position is hopeless\n");
...
revise_semeai(int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	TRACE_T("revise_semeai: changed status of dragon %1m from DEAD to UNKNOWN\n",
	      pos);
    }
  }
