...
moves_connection_one_move(int *moves, int str1, int str2, zone *zn)
{
  ...
  if (board[str1] == EMPTY_T || board[str2] == EMPTY_T)
    return 0;
...
recursive_transitivity(int str1, int str2, int str3, int *move)
{
  ...
  if (board[str1] == EMPTY_T || board[str2] == EMPTY_T || board[str3] == EMPTY_T) {
    SGFTRACE2(PASS_MOVE, 0, "one string already captured");
    return 0;
  }
		 ...
		 int moves[MAX_MOVES], float *total_distance)
{
  ...
  for (k = 0; k < conn1.queue_end; k++)
	...
	if (goal[stones[i]]) {
	  ...
	  TRACE_T("%oUsing %1m as secondary target.\n", str2);
...
spread_connection_distances(int color, struct connection_data *conn)
{
  ...
  while (conn->queue_start < conn->queue_end || conn->heap_size > 0) {
	...
	if (board[apos] == EMPTY_T && is_suicide(apos, other))
	 ...
	 */
	if (board[apos] == color && board[bpos] == EMPTY_T
	...
	if (board[gpos] == EMPTY_T
	  ...
	  if (board[bpos] == EMPTY_T
	    ...
	    if (board[apos] == EMPTY_T && board[fpos] == EMPTY_T) {
	      ENQUEUE_STONE(conn, pos, epos, distance + 0.2, 0.2,
			    apos, fpos);
	    }
	    else if (board[apos] == EMPTY_T && board[fpos] != EMPTY_T) {
	      ENQUEUE_STONE(conn, pos, epos, distance + 0.2, 0.2,
			    apos, NO_MOVE);
	    }
	    else if (board[apos] != EMPTY_T && board[fpos] == EMPTY_T) {
	  ...
	  if (board[ipos] == EMPTY_T
	    ...
	    if (board[hpos] == EMPTY_T && board[jpos] == EMPTY_T) {
	      ENQUEUE_STONE(conn, pos, epos, distance + 0.2, 0.2,
			    hpos, jpos);
	    }
	    else if (board[hpos] == EMPTY_T && board[jpos] != EMPTY_T) {
	      ENQUEUE_STONE(conn, pos, epos, distance + 0.2, 0.2,
			    hpos, NO_MOVE);
	    }
	    else if (board[hpos] != EMPTY_T && board[jpos] == EMPTY_T) {
	 ...
	 * Case 7. "a" is empty.
	 */
	if (board[apos] == EMPTY_T && conn->distances[apos] > distance + 0.6) {
	 ...
	 */
	if (board[bpos] == EMPTY_T
	    && board[apos] == EMPTY_T && board[gpos] == EMPTY_T
