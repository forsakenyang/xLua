...
enlarge_goal(char goal[BOARDMAX])
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (board[pos] == EMPTY_T && !goal[pos]) {
      int k;
      int goal_neighbors = 0;
      for (k = 0; k < 4; k++)
	if (board[pos + delta[k]] == EMPTY_T && goal[pos + delta[k]] == 1)
	  goal_neighbors++;
      if (goal_neighbors >= 2)
	goal[pos] = 2;
    }
    		     ...
    		     const char goal[BOARDMAX], char smaller_goal[BOARDMAX])
{
  ...
  for (k = 0; k < conn->queue_end; k++) {
    ...
    for (j = 0; j < 4; j++)
	  ...
	  && goal[pos + delta[j]]
	  && (board[pos] == EMPTY_T || goal[pos] == OTHER_COLOR(owner)))
	goal_neighbors++;
#if 0
    if (goal_neighbors > 2
    		      ...
    		      int color_to_move)
{
  ...
  DEBUG_T(DEBUG_BREAKIN, "Trying to break in from %1m to:\n", str);
  ...
  while ((color_to_move == board[str]
    ...
    if (ON_BOARD(move) && goal[move]) {
      ...
      DEBUG_T(DEBUG_TERRITORY, "Erasing territory at %1m -a.\n", move);
    }
	...
	DEBUG_T(DEBUG_TERRITORY, "Erasing territory at %1m -b.\n", pos);
	...
	*num_non_territory = k;
    ...
    compute_smaller_goal(OTHER_COLOR(board[str]), color_to_move,
			 &conn, goal, smaller_goal);
    DEBUG_T(DEBUG_BREAKIN, "Now trying to break to smaller goal:\n", str);
    if (debug & DEBUG_BREAKIN)
      goaldump(smaller_goal);
