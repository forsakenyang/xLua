...
move_connects_strings(int pos, int color)
{
  ...
  for (k = 0; k < 4; k++) {
    ...
    if (!ON_BOARD(ii) || board[ii] == EMPTY_T)
      continue;
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
	...
	if (board[aa] == color
	  ...
	  if (acode < worm[aa].attack_codes[0]) {
		...
		TRACE_T("\n");
	      TRACE_T("%ofound extra point of defense of %1m at %1m code %d\n",
	...
	if (board[aa] == other
	  ...
	  if (dcode < worm[aa].defense_codes[0]) {
	    ...
	    if (attack_works) {
	      if (!cursor_at_start_of_line)
		TRACE_T("\n");
	      TRACE_T("%ofound extra point of attack of %1m at %1m code %d\n",
    ...
    for (dist = 0; dist <= maximum_distance; dist++)
      for (x = x_min_dragon[k]-dist; x <= x_max_dragon[k]+dist; x++)
        for (y = y_min_dragon[k]-dist; y <= y_max_dragon[k]+dist; y++) {
          ...
          if (ON_BOARD2(x, y) && ON_BOARD1(pos) && (board[pos] == EMPTY_T)) {
            int a, b;
...
induce_secondary_move_reasons(int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	for (i = 0; i < DRAGON2(aa).neighbors; i++) {
	  ...
	  if (dragon[bb].color == color && worm[bb].attack_codes[0] == 0
	    ...
	    do_find_more_owl_attack_and_defense_moves(color, pos,
						      ...
						      bb);
	    DEBUG_T(DEBUG_MOVE_REASONS, "Strategic defense at %1m induced for %1m due to owl attack on %1m\n",
		  pos, bb, aa);
	  }
	}
      }
	...
	int pos2;
	for (pos2 = BOARDMIN; pos2 < BOARDMAX; pos2++) {
	  if (ON_BOARD(pos2) && board[pos2] == EMPTY_T
		...
		if (trymove(pos, color, "induce_secondary_move_reasons-B",
		  ...
		  if (!disconnect(pos3, worm1, NULL)) {
		    ...
		    DEBUG_T(DEBUG_MOVE_REASONS, "Connection at %1m induced for %1m/%1m due to connection at %1m/%1m\n",
...
adjacent_to_nondead_stone(int pos, int color)
{
  ...
  if (trymove(pos, color, NULL, EMPTY_T)) {
    for (k = 0; k < 12; k++) {
      int pos2;
      if (k < 8)
	pos2 = pos + delta[k];
			   ...
			   int disable_delta_territory_cache)
{
  ...
  for (k = 0; k < MAX_REASONS; k++) {
	...
	this_value *= 0.3;
	TRACE_T("  %1m: -%f - attack on worm %1m only with good ko\n",
	      pos, this_value, aa);
      }	
      else if (move_reasons[r].type == ATTACK_MOVE_BAD_KO) {
	this_value *= 0.5;
	TRACE_T("  %1m: -%f - attack on worm %1m only with bad ko\n",
	      pos, this_value, aa);
      }
	...
	this_value *= 0.3;
	TRACE_T("  %1m: -%f - defense of worm %1m with good ko\n",
	      pos, this_value, aa);
      }	
      else if (move_reasons[r].type == DEFEND_MOVE_BAD_KO) {
	this_value *= 0.5;
	TRACE_T("  %1m: -%f - defense of worm %1m with bad ko\n",
	      pos, this_value, aa);
      }	
	...
	TRACE_T("  %1m: -%f - suspected ineffective defense of worm %1m\n",
	      pos, this_value, aa);
	tot_value -= this_value;
      }
	...
	if (!is_known_good_attack_threat(pos, aa)
	  ...
	  if (trymove(defense_move, other,
	    ...
	    if (board[pos] == EMPTY_T || attack(pos, NULL) != 0) {
	      popgo();
	      popgo();
	      break;
	    }
	  ...
	  for (s = 0; s < num_adj; s++) {
		...
		if (!attack(aa, NULL)
		    && (board[pos] == EMPTY_T || attack(pos, NULL) != 0)) {
		  popgo();
		  bad_followup = 1;
		  break;
		}
	...
	if (adjusted_value > 0.0) {
	  ...
	  TRACE_T("  %1m:   %f (followup) - threatens to capture %1m\n",
	...
	popgo();
      ...
      TRACE_T("  %1m:   %f (followup) - threatens to defend %1m\n",
    ...
    case SEMEAI_THREAT:
      ...
      TRACE_T("  %1m: %f (followup) - threatens to win semeai for %1m\n",
	...
	TRACE_T("  %1m: -0.5 - penalty for ko stone %1m (workaround)\n",
	      pos, aa);
	tot_value -= 0.5;
      }
      ...
      TRACE_T("  %1m: owl attack/defend for %1m\n", pos, aa);
	...
	this_value = 0.0;
	       ...
	       || move_reasons[r].type == OWL_DEFEND_MOVE_GOOD_KO) {
	this_value *= 0.3;
	TRACE_T("  %1m: -%f - owl attack/defense of %1m only with good ko\n",
	      pos, this_value, aa);
      }	
      else if (move_reasons[r].type == OWL_ATTACK_MOVE_BAD_KO
	       || move_reasons[r].type == OWL_DEFEND_MOVE_BAD_KO) {
	this_value *= 0.5;
	TRACE_T("  %1m: -%f - owl attack/defense of %1m only with bad ko\n",
	      pos, this_value, aa);
      }
	...
	TRACE_T("  %1m: -%f - suspected ineffective owl attack of worm %1m\n",
	      pos, this_value, aa);
	tot_value -= this_value;
      }
	...
	if (value > 0.0) {
	  ...
	  TRACE_T("  %1m: %f (followup) - threatens to owl attack %1m\n",
		pos, value, aa);
	}
      }
      break;
    ...
    case OWL_DEFEND_THREAT:
      ...
      add_followup_value(pos, 2 * dragon[aa].effective_size);
      TRACE_T("  %1m: %f (followup) - threatens to owl defend %1m\n",
	    pos, 2 * dragon[aa].effective_size, aa);
      break;
	...
	TRACE_T("  %1m: %f - attack last move played, although it seems dead\n",
	      pos, this_value);
	...
	TRACE_T("  %1m: %f - attack %1m, although it seems dead, as we are ahead\n",
	      pos, this_value, aa);
	tot_value += this_value * attack_dragon_weight;
      }
      else {
	...
	if (board[aa] == color)
	  TRACE_T("  %1m: %f (reverse followup) - prevent threat to attack %1m\n",
		pos, 2 * dragon[aa].effective_size, aa);
	else 
	  TRACE_T("  %1m: %f (reverse followup) - prevent threat to defend %1m\n",
		pos, 2 * dragon[aa].effective_size, aa);
      }
      break;
    ...
    case MY_ATARI_ATARI_MOVE:
      ...
      tot_value += this_value;
      TRACE_T("  %1m: %f - combination attack kills one of several worms\n",
	    pos, this_value);
      break;
    ...
    case YOUR_ATARI_ATARI_MOVE:
      ...
      tot_value += this_value;
      TRACE_T("  %1m: %f - defends against combination attack on several worms\n",
	    pos, this_value);
      break;
    }
  }
  ...
  if (move[pos].move_safety == 1 && safe_move(pos, color) == WIN) {
    ...
    if (0)
      TRACE_T("  %1m: is a safe move\n", pos);
  }
  else {
    TRACE_T("  %1m: not a safe move\n", pos);
    safe_stones[pos] = 0;
    strength[pos] = 0.0;
  }
  ...
  if (does_block
	...
	use_optimistic_territory = (size_of_biggest_critical_dragon() <= 5);  
      ...
      if (this_value != 0.0)
	TRACE_T("%1m: %f - change in territory\n", pos, this_value);
      else
	DEBUG_T(DEBUG_MOVE_REASONS, "%1m: 0.00 - change in territory\n", pos);
      move[pos].influence_followup_value
	= influence_delta_territory(&move_influence, &followup_influence,
      ...
      if (this_value != 0.0)
	TRACE_T("%1m: %f - change in territory (cached)\n", pos, this_value);
      else
	DEBUG_T(DEBUG_MOVE_REASONS,
  ...
  if (tot_value < move[pos].min_territory
    ...
    TRACE_T("  %1m:   %f - revised to meet minimum territory value\n", 
	  pos, tot_value);
  }
  if (tot_value > move[pos].max_territory) {
    tot_value = move[pos].max_territory;
    TRACE_T("  %1m:   %f - revised to meet maximum territory value\n",
	  pos, tot_value);
  }
...
estimate_strategical_value(int pos, int color, float our_score)
{
  ...
  for (k = 0; k < MAX_REASONS; k++) {
	...
	TRACE_T("  %1m: %f - either attacks %1m (%f) or attacks %1m (%f)\n",
	...
	TRACE_T("  %1m: %f - both defends %1m (%f) and defends %1m (%f)\n",
	      pos, this_value, aa, aa_value, bb, bb_value);
  ...
  for (aa = BOARDMIN; aa < BOARDMAX; aa++) {
	...
	&& !owl_move_reason_known(pos, aa)) {
      DEBUG_T(DEBUG_MOVE_REASONS, "  %1m: 0.0 - disregarding strategic effect on %1m (critical dragon)\n",
	    pos, aa);
      continue;
    }
	...
	&& (attack_move_reason_known(pos, aa)
	    || defense_move_reason_known(pos, aa))) {
      TRACE_T("  %1m:   %f - %1m strategic value already counted - A.\n",
	...
	|| move_reason_known(pos, SEMEAI_MOVE, aa)) {
      ...
      if (excess_value > 0.0) {
	TRACE_T("  %1m: %f - strategic bonus for %1m\n", pos, excess_value, aa);
	tot_value += excess_value;
      }
      else {
	TRACE_T("  %1m:   %f - %1m strategic value already counted - B.\n",
	      pos, dragon_value[aa], aa);
      }
    ...
    TRACE_T("  %1m: %f - strategic effect on %1m\n",
	  pos, dragon_value[aa], aa);
  ...
  if (this_value > 0.0) {
    TRACE_T("  %1m: %f - strategic penalty, considered as invasion.\n",
	  pos, -this_value);
    tot_value -= this_value;
  }
		   ...
		   float our_score)
{
  ...
  if (tot_value > 0.0) {
    ...
    followup_value = move[pos].followup_value
		     + move[pos].influence_followup_value;
    TRACE_T("  %1m:   %f - total followup value, added %f as territorial followup\n",
    ...
    if (tot_value >= 0.5) {
      ...
      if (contribution != 0.0) {
	TRACE_T("  %1m: %f - added due to followup (%f) and reverse followup values (%f)\n",
	...
	&& (move_reason_known(pos, CONNECT_MOVE, -1)
      ...
      TRACE_T("  %1m: %f - cut/connect dame bonus\n", pos,
	    tot_value - old_tot_value);
    }
  }
  else {
  ...
  if (pure_threat_value > 0.0 
    ...
    if (new_tot_value > tot_value) {
      TRACE_T("  %1m: %f - carry out threat or defend against threat\n",
  ...
  if (tot_value < move[pos].min_value
    ...
    TRACE_T("  %1m:   %f - minimum accepted value\n", pos, tot_value);
  }
  
  if (tot_value > move[pos].max_value) {
    tot_value = move[pos].max_value;
    TRACE_T("  %1m:   %f - maximum accepted value\n",
          pos, tot_value);
  }
  ...
  if (tot_value > 0
    ...
    TRACE_T("Move generation values %1m to %f\n", pos, tot_value);
...
value_moves(int color, float pure_threat_value, float our_score)
{
  ...
  for (n = 0; n < board_size; n++)
	...
	move[pos].value = 0.0;
	TRACE_T("Move at %1m wasn't legal.\n", pos);
      }
    }
}
...
reevaluate_ko_threats(int ko_move, int color, float ko_value)
{
  ...
  TRACE_T("Reevaluating ko threats.\n");
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (!find_defense(ko_stone, &opp_ko_move))
	  ...
	  if (whose_area(OPPOSITE_INFLUENCE(color), pos) != EMPTY_T)
	  ...
	  if (trymove(opp_ko_move, OTHER_COLOR(color),
	    ...
	    case ATTACK_THREAT:
	      ...
	      break;
	    case DEFEND_THREAT:
	      threat_does_work = (board[what] != EMPTY_T
	       ...
	       int allowed_moves[BOARDMAX])
{
  ...
  while (!good_move_found) {
	...
	if (is_legal(pos, color) || is_illegal_ko_capture(pos, color)) {
	  ...
	  TRACE_T("Move at %1m would be suicide.\n", pos);
	  remove_top_move(pos);
	  move[pos].value = 0.0;
    ...
    if (bestval > 0.0 && is_illegal_ko_capture(best_move, color)) {
      TRACE_T("Move at %1m would be an illegal ko capture.\n", best_move);
	...
	if (blunder_size > 0.0) {
	  TRACE_T("Move at %1m is a blunder, subtracting %f.\n", best_move,
		blunder_size);
	  remove_top_move(best_move);
	  move[best_move].value -= blunder_size;
	  move[best_move].final_value -= blunder_size;
	  TRACE_T("Move at %1m is now valued %f.\n", best_move,
...
choose_strategy(int color, float our_score, float game_status)
{
  ...
  TRACE_T("  Game status = %f (0.0 = start, 1.0 = game over)\n", game_status);
  ...
  if (cosmic_gnugo) {
    ...
    if (game_status > 0.65 && our_score > 15.0) {
      ...
      followup_weight       = 1.1;
      TRACE_T("  %s is leading, using conservative settings.\n",
      ...
      TRACE_T("  %s is not winning enough, using aggressive settings.\n", 
	    color == WHITE ? "White" : "Black");
    }
  }
}
