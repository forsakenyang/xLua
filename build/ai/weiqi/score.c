...
#define ARRAYSIZE_T MAX_BOARD*MAX_BOARD
...
dilate_erode(int dilations, int erosions, int gb[BOARDMAX], int color)
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
	...
	&& DRAGON2(ii).safety != INESSENTIAL) {
      critical_found = 1;
      DEBUG_T(DEBUG_SCORING, "critical dragon found at %1m\n", ii);
    }
...
estimate_score(float *upper, float *lower)
{
  ...
  for (i = 0; i < board_size; i++) {
    ...
    if (chinese_rules) {
      DEBUG_T(DEBUG_SCORING, "in row %d, white area=%d, black=%d\n",
	    board_size - i, white_area_in_row, black_area_in_row);
    }
    else {
      DEBUG_T(DEBUG_SCORING, "in row %d, white territory=%d, black=%d\n",
