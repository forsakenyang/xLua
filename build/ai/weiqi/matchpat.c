...
static const int val_mask[2][8] = {
  { EMPTY_T, BLACK, WHITE,  0,     0,   EMPTY_T, EMPTY_T, EMPTY_T},  /* BLACK */ 
  { EMPTY_T, WHITE, BLACK,  0,     0,   EMPTY_T, EMPTY_T, EMPTY_T}   /* WHITE */
};
...
static void
fixup_patterns_for_board_size(struct pattern_b *pattern)
{
  for (; pattern->patn; ++pattern)
...
do_matchpat(int anchor, matchpat_callback_fn_ptr callback, int color,
	    struct pattern_b *pattern, void *callback_data,
	    char goal[BOARDMAX]) 
{
  ...
  do {
	...
	for (k = 0; k < pattern->patlen; ++k) { /* match each point */
	  ...
	  if (goal != NULL && board[pos] != EMPTY_T) {
	    if (goal[pos])
	      found_goal = 1;
	...
	if ((goal != NULL) && !found_goal)
	  goto match_failed;
	if ((goal != NULL) && ((pattern->class_T) & CLASS_C) && !found_nongoal)
	  goto match_failed;
                     ...
                     struct rec_data *pdata)
{
  ...
  while (tnl) {
    ...
    if (ON_BOARD2(x, y)) {
      ...
      if ((att == EMPTY_T && point_color == EMPTY_T)
...
static const int convert[3][4] = {
  {-1, -1, -1, -1},		/* not used */
  {EMPTY_T, WHITE, BLACK, OUT_BOARD},	/* WHITE */
  {EMPTY_T, BLACK, WHITE, OUT_BOARD}	/* BLACK */
};
...
static void check_pattern_light(int anchor, 
				matchpat_callback_fn_ptr callback,
				int color, struct pattern_b *pattern, int ll,
				void *callback_data,
				char goal[BOARDMAX],
                                int anchor_in_goal);
...
dfa_match_init(void)
{
  ...
  if (owl_vital_apat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "owl_vital_apat --> using dfa\n");
  if (owl_attackpat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "owl_attackpat --> using dfa\n");
  if (owl_defendpat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "owl_defendpat --> using dfa\n");
  if (pat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "pat --> using dfa\n");
  if (attpat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "attpat --> using dfa\n");
  if (defpat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "defpat --> using dfa\n");
  if (endpat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "endpat --> using dfa\n");
  if (conn_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "conn --> using dfa\n");
  if (influencepat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "influencepat --> using dfa\n");
  if (barrierspat_db.pdfa != NULL)
    DEBUG_T(DEBUG_MATCHER, "barrierspat --> using dfa\n");
...
check_pattern_light(int anchor, matchpat_callback_fn_ptr callback, int color,
		    struct pattern_b *pattern, int ll, void *callback_data,
		    char goal[BOARDMAX], int anchor_in_goal)
{
  int k;			/* Iterate over elements of pattern */
  int found_goal = 0;
  ...
  for (k = 0; k < pattern->patlen; k++) {
    ...
    if ((pattern->class_T & class_mask[dragon[pos].status][board[pos]]) != 0)
      goto match_failed;
  ...
  /* We jump to here as soon as we discover a pattern has failed. */
 match_failed:
  DEBUG_T(DEBUG_MATCHER, "end of pattern '%s', rotation %d at %1m\n---\n",
	pattern->name, ll, anchor);
