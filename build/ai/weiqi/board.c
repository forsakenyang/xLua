...
#define UNMARKED_LIBERTY(pos) \
  (board[pos] == EMPTY_T && ml[pos] != liberty_mark)
...
clear_board(void)
{
  ...
  memset(board, EMPTY_T, sizeof(board));
  memset(initial_board, EMPTY_T, sizeof(initial_board));
...
play_move(int pos, int color)
{
  ...
  ASSERT1(pos == PASS_MOVE || board[pos] == EMPTY_T, pos);
  ASSERT1(komaster == EMPTY_T && kom_pos == NO_MOVE, pos);
 ...
 * 
 * 1. Komaster is EMPTY_T.
 * 1a) Unconditional ko capture is allowed.
 *       Komaster remains EMPTY_T if previous move was not a ko capture.
 ...
 *    filled then the komaster reverts to EMPTY_T.
 *
 * 5. Komaster is WEAK_KO:
 * 5a) After a non-ko move komaster reverts to EMPTY_T.
		 ...
		 int *is_conditional_ko, int consider_conditional_ko)
{
   ...
   * resolved. If that is the case, we revert komaster to EMPTY_T.
   *
  ...
  *is_conditional_ko = 0;
  ...
  if (!trymove(pos, color, message, str)) {
    ...
    *is_conditional_ko = 1;
    ...
    if (komaster == EMPTY_T || komaster == WEAK_KO) {
      PUSH_VALUE(komaster);
      PUSH_VALUE(kom_pos);
      komaster = color;
      kom_pos = kpos;
  ...
  if (komaster == other) {
    ...
    /* We can reach here when komaster is EMPTY_T or WEAK_KO. If previous
 ...
 * may use  if (board[libs[k]] != EMPTY_T)  construction.
 *
 * If you want the number or the locations of all liberties, however
 * many they are, you should pass MAXLIBS as the value for maxlib and
 * allocate space for libs[] accordingly.
...
dump_incremental_board(void)
{
  ...
  for (s = 0; s < next_string; s++) {
    if (board[string[s].origin] == EMPTY_T)
      continue;
