...
struct pattern_b; /* forward reference to keep gcc happy */
...
typedef int (*pattern_helper_fn_ptr)(struct pattern_b *, int rotation,
				     int move, int color);
 ...
 */
struct pattern_b {
  struct patval_a *patn;  /* array of elements */
  ...
  unsigned int class_T;   /* classification of pattern */
...
#define DECLARE(x) int x(struct pattern_b *pattern, int transformation, int move, int color)
...
struct corner_pattern {
  ...
  unsigned int class_T;	/* Pattern class. */
