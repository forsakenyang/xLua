		       ...
		       int your_ndragons)
{
   ...
   */
  if (class_T & CLASS_J) {
    TRACE_T("...joseki standard move\n");
    add_expand_territory_move(move);
    TRACE_T("...expands territory\n");
    add_expand_moyo_move(move);
    TRACE_T("...expands moyo\n");
    set_minimum_move_value(move, J_VALUE);
    TRACE_T("... minimum move value %f\n", J_VALUE);
  }

  /* Class `j' and `t' patterns are treated similarly. */
  if (class_T & (CLASS_j | CLASS_t)) {
    float min_value;
    float shape_value = 0.0;

    if (class_T & CLASS_j) {
      min_value = j_VALUE;
      TRACE_T("...less urgent joseki move\n");
      add_expand_territory_move(move);
      TRACE_T("...expands territory\n");
      add_expand_moyo_move(move);
      TRACE_T("...expands moyo\n");
    ...
    if ((board_size >= 17) && (class_T & CLASS_F)) {
      ...
      TRACE_T("...move value %f (shape %f)\n", min_value, shape_value);
    }
    else
      TRACE_T("...minimum move value %f (shape %f)\n", min_value, shape_value);
 ...
 */
static void
shapes_callback(int anchor, int color, struct pattern_b *pattern, int ll,
		void *data)
{
  ...
  unsigned int class_T = pattern->class_T;

  /* Don't accept fuseki marked patterns while scoring. */
  if (doing_scoring && (class_T & CLASS_F))
    return;
   ...
   * in the pattern.
   */
  if ((class_T & (CLASS_B | CLASS_C | CLASS_c | CLASS_a | CLASS_d | CLASS_O
  ...
  if (pattern->helper) {
    ...
    if (accepted) {
      DEBUG_T(DEBUG_HELPER, "helper likes pattern '%s' at %1m\n",
	    pattern->name, move);
    }
    else {
  ...
  if (allpats || verbose) {
    TRACE_T("pattern '%s'+%d matched at %1m\n", pattern->name, ll, move);
  }
  ...
  if (class_T & CLASS_C) {
	...
	if (disconnect(my_strings[k], my_strings[l], NULL)
	  ...
	  TRACE_T("...connects strings %1m, %1m\n",
		my_strings[k], my_strings[l]);
	}
      }
  }
  ...
  if (class_T & CLASS_c) {
    for (k = 0; k < my_ndragons; k++) {
      add_strategical_defense_move(move, my_dragons[k]);
      TRACE_T("...strategical defense (weak connection) of %1m\n",
	    my_dragons[k]);
    }
    add_shape_value(move, 1);
    TRACE_T("...shape value 1\n");
  }

  /* Pattern class b is obsolete in the pattern databases handled here. */
  gg_assert(!(class_T & CLASS_b));
  ...
  if (class_T & CLASS_W) {
    TRACE_T("...worthwhile threat move\n");
    add_worthwhile_threat_move(move);
  }

  handle_joseki_patterns(pattern->attributes, class_T, move,
