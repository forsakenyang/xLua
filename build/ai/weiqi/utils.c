...
test_symmetry_after_move(int move, int color, int strict)
{
  ...
  for (pos = BOARDMIN; pos < MIRROR_MOVE(pos); pos++) {
    ...
    if (sum != EMPTY_T + EMPTY_T && sum != BLACK + WHITE) {
      if (strict || sum == EMPTY_T + WHITE || sum == EMPTY_T + BLACK) {
      result = 0;
      break;
      }
    }
...
play_attack_defend2_n(int color, int do_attack, int num_moves, ...)
{
  ...
  if (do_attack) {
    if (board[ypos] == EMPTY_T || board[zpos] == EMPTY_T)
      success = WIN;
    else
	     ...
	     char safe_stones[BOARDMAX])
{
  ...
  TRACE_T("Checking safety of a %s move at %1m\n", color_to_string(color), move);
  ...
  if (atari) {
    ...
    TRACE_T("Combination attack appears.\n");
		   ...
		   float *return_value, int save_verbose)
{
  ...
  for (k = 0; k < 4; k++) {
	...
	for (r = 0; r < DRAGON2(bpos).neighbors; r++) {
      ...
      if (acode == 0) {
	verbose = save_verbose;
	TRACE_T("Dragon at %1m becomes attackable.\n", bpos);
			...
			float *return_value, int save_verbose)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	find_defense(pos, defense_point);
      verbose = save_verbose;
      TRACE_T("After %1m Worm at %1m becomes attackable.\n", move, pos);
      verbose = current_verbose;
	...
	*return_value += 2 * worm[pos].effective_size;
	defense_effective = 1;
	verbose = save_verbose;
	TRACE_T("After %1m worm at %1m becomes defendable - A.\n", move, pos);
	verbose = current_verbose;
      }
	...
	for (k = 0; k < num_adj; k++) {
	  ...
	  if (resulta != 0) {
	    *return_value += 2 * worm[pos].effective_size;
	    defense_effective = 1;
	    verbose = save_verbose;
	    TRACE_T("After %1m worm at %1m becomes defendable - B.\n",
		  move, pos);
	    verbose = current_verbose;
	    break;
	  }
	...
	if (attack(pos, &dpos)) {
	  *defense_point = dpos;
	  ...
	  verbose = save_verbose;
	  TRACE_T("No attack found (unexpectedly) on %1m after move at %1m.\n",
		pos, move);
	  verbose = current_verbose;
	}
      }
  ...
  if (liberties == 2) {
	...
	*defense_point = libs[0];
      *return_value += d_a_blunder_size;
      verbose = save_verbose;
      TRACE_T("Double threat appears at %1m.\n", libs[0]);
      verbose = current_verbose;
    }
...
double_atari(int move, int color, float *value, char safe_stones[BOARDMAX])
{
  ...
  for (k = 4; k < 8; k++) {
	...
	&& trymove(move, color, "double_atari", NO_MOVE)) {
      if (countlib(move) > 1
	  && (BOARD(m, n+dn) == EMPTY_T || BOARD(m+dm, n) == EMPTY_T 
...
unconditional_life(int unconditional_territory[BOARDMAX], int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    for (r = 0; r < 4; r++) {
	  ...
	  || board[pos + up + up] != EMPTY_T
	  || board[pos - up] != EMPTY_T)
	continue;
      for (k = 0; k < 2; k++) {
	if (k == 1)
	  right = -right;
	if (board[pos + right] != EMPTY_T || board[pos + up - right] != EMPTY_T)
	  continue;
	if (board[pos - right] == EMPTY_T
	    && trymove(pos - right, other, "unconditional_life", pos))
	  locally_played_moves++;
	if (board[pos + up + right] == EMPTY_T
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (!potential_sekis[pos] || board[pos] == EMPTY_T)
      continue;
    /* Play as many liberties as we can. */
    liberties = findlib(pos, MAXLIBS, libs);
    for (k = 0; k < liberties; k++) {
