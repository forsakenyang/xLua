...
static void attack_callback(int anchor, int color,
			    struct pattern_b *pattern, int ll, void *data);
...
make_worms(void)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	worm[pos].cutstone = 1;
      ...
      DEBUG_T(DEBUG_WORMS, "Worm at %1m has w1 %1m and w2 %1m, cutstone %d\n",
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	&& (worm[lunch].attack_codes[0] == WIN
      ...
      DEBUG_T(DEBUG_WORMS, "lunch found for %1m at %1m\n", pos, lunch);
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	DEBUG_T(DEBUG_WORMS, "Worm %1m identified as inessential.\n", pos);
...
find_worm_attacks_and_defenses()
{
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++) {
    ...
    TRACE_T("considering attack of %1m\n", str);
    ...
    if (acode != 0) {
      DEBUG_T(DEBUG_WORMS, "worm at %1m can be attacked at %1m\n",
	    str, attack_point);
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++) {
    ...
    if (worm[str].attack_codes[0] != 0) {
      ...
      TRACE_T("considering defense of %1m\n", str);
      dcode = find_defense(str, &defense_point);
      if (dcode != 0) {
	TRACE_T("worm at %1m can be defended at %1m\n", str, defense_point);
	...
	if (!liberty_of_string(attack_point, str))
	  if (trymove(attack_point, worm[str].color, "make_worms", NO_MOVE)) {
	    ...
	    if (acode != WIN) {
	      ...
	      TRACE_T("worm at %1m can be defended at %1m with code %d\n",
  ...
  for (str = BOARDMIN; str < BOARDMAX; str++) {
	...
	if (trymove(pos, other, "make_worms", str)) {
	  if (board[str] == EMPTY_T || attack(str, NULL)) {
	    if (board[str] == EMPTY_T)
	      dcode = 0;
	    else
...
{
  str = worm[str].origin;
  DEBUG_T(DEBUG_WORMS, "change_attack: %1m %1m %d\n", str, move, acode);
...
worm_reasons(int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (!ON_BOARD(pos) || board[pos] == EMPTY_T)
      continue;
...
genus(int str)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (ON_BOARD(pos)
	&& !mg[pos]
	&& (board[pos] == EMPTY_T || !is_same_worm(pos, str))) {
...
markcomponent(int str, int pos, int mg[BOARDMAX])
{
  ...
  for (k = 0; k < 4; k++) {
    ...
    if (ON_BOARD(apos)
	&& mg[apos] == 0
	&& (board[apos] == EMPTY_T || !is_same_worm(apos, str)))
...
examine_cavity(int pos, int *edge)
{
  ...
  *edge = 0;
   ...
   */
  gg_assert(border_color == EMPTY_T
	    && ((pos == NO_MOVE
	       ...
	       int *border_color, int *edge, int str)
{
  ...
  if (is_edge_vertex(pos) && board[pos] == EMPTY_T) 
    (*edge)++;
 ...
 */
static void
attack_callback(int anchor, int color, struct pattern_b *pattern, int ll,
		void *data)
{
  int move;
  int k;
...
static void
defense_callback(int anchor, int color, struct pattern_b *pattern, int ll,
		 void *data)
{
  int move;
  int k;
