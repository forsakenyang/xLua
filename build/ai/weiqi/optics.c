	     ...
	     int owl_call)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    if (board[pos] == EMPTY_T || !lively[pos]) {
			...
			int first_time)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    if (own) {
       ...
       */
      if (first_time) {
        if (board[pos] == EMPTY_T && (false_margin(pos, color, lively)
      ...
      else if (board[pos] != EMPTY_T || !false_margins[pos]) {
        domain[pos] = 1;
        influence[pos] = 1;
      }
    }
...
false_margin(int pos, int color, int lively[BOARDMAX])
{
  ...
  if (potential_false_margin && safe_move(pos, other) == 0) {
    DEBUG_T(DEBUG_EYES, "False margin for %C at %1m.\n", color, pos);
    return 1;
  }
		 ...
		 int dragons[], int max_dragons)
{
  ...
  DEBUG_T(DEBUG_MISCELLANEOUS, "find_eye_dragons: %1m %C\n", origin, eye_color);
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	  ...
	  int pos)
{
  ...
  for (pos2 = BOARDMIN; pos2 < BOARDMAX; pos2++) {
	...
	gprintf("%1m (XH) (topological eye value = %f)\n", pos2,
      ...
      gprintf("%1m (X)\n", pos2);
    else if (eye[pos2].marginal && board[pos2] == EMPTY_T)
      gprintf("%1m (!)\n", pos2);
    else if (is_halfeye(heye, pos2) && board[pos2] == EMPTY_T) {
      if (heye[pos2].value == 3.0)
	gprintf("%1m (H)\n", pos2);
      else
			 ...
			 struct half_eye_data heye[BOARDMAX])
{
  ...
  if (read_eye(pos, attack_point, defense_point, value,
    ...
    *pessimistic_min = min_eyes(value) - margins;
    ...
    DEBUG_T(DEBUG_EYES, "  graph matching - %s, pessimistic_min=%d\n",
   ...
   */
    ...
    DEBUG_T(DEBUG_EYES, "  guess_eye - %s, pessimistic_min=%d\n",
	  eyevalue_to_string(value), *pessimistic_min);
  }

  if (*pessimistic_min < 0) {
    *pessimistic_min = 0;
    DEBUG_T(DEBUG_EYES, "  pessimistic min revised to 0\n");
  }
  ...
  if (*pessimistic_min < 1 && interior_stones >= 2) {
    *pessimistic_min = 1;
    DEBUG_T(DEBUG_EYES, "  pessimistic min revised to 1 (interior stones)\n");
  }
  ...
  if (attack_point
	...
	if (eye[pos2].marginal && board[pos2] == EMPTY_T) {
  ...
  if (effective_eyesize > 3) {
    ...
    *pessimistic_min = 1;
	...
	|| (margins > 0 && effective_eyesize > 9)) {
      ...
      DEBUG_T(DEBUG_EYES, "size: %d(%d), threshold: %d, bulk score: %d\n",
	      ...
	      struct vital_points *vp)
{
  ...
  for (graph = 0; graphs[graph].vertex != NULL; graph++) {
	...
	DEBUG_T(DEBUG_EYES, "  vital points: %1m (attack) %1m (defense)\n",
	      *attack_point, *defense_point);
	DEBUG_T(DEBUG_EYES, "  pattern matched:  %d\n", graphs[graph].patnum);
	
      }
      TRACE_T("eye space at %1m of type %d\n", pos, graphs[graph].patnum);
	      ...
	      struct half_eye_data heye[BOARDMAX])
{
  ...
  DEBUG_T(DEBUG_EYES, "false eye found at %1m\n", pos);
  ...
  *defense_point = NO_MOVE;
   ...
   */
   if (my_eye[pos].color == color
       ...
       && !(board[pos] == EMPTY_T && does_capture_something(pos, other)))
    return 0.0;
...
test_eyeshape(int eyesize, int *eye_vertices)
{
   ...
   */
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (mx[pos] == 1 || board[pos] != EMPTY_T || liberty_of_string(pos, str))
      continue;
    for (k = 0; k < 8; k++) {
      if (ON_BOARD(pos + delta[k])
  ...
  for (n = 0; n < N; n++) {
    ...
    if (attack_code == 0) {
       ...
       */
      for (k = 0; k < eyesize; k++) {
	if (board[eye_vertices[k]] == EMPTY_T
	...
	if (board[defense_point] != EMPTY_T
...
eyegraph_order_moves(int num_vertices, int *vertices, int color_to_move, int *moves)
{
  ...
  for (k = 0; k < num_vertices; k++) {
	...
	break;
      ...
      if (k == num_vertices - 2 && board[vertices[num_vertices - 3]] == EMPTY_T)
	continue;
    }
	   ...
	   int distance)
{
  ...
  mx[pos + up + right] = EMPTY_T;
  mx[pos + 2 * up + 2 * right] = EMPTY_T;
