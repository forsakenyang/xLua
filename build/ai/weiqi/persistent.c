			    ...
			    const char goal[BOARDMAX], int dummy)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (IS_STONE(board[pos]) || active[pos] != 0) 
      continue;
    if ((board[SOUTH(pos)] == EMPTY_T && active[SOUTH(pos)] == 2)
	|| (board[WEST(pos)] == EMPTY_T && active[WEST(pos)] == 2)
	|| (board[NORTH(pos)] == EMPTY_T && active[NORTH(pos)] == 2)
	|| (board[EAST(pos)] == EMPTY_T && active[EAST(pos)] == 2))
      active[pos] = 3;
  }
			   ...
			   int m, int n, float contribution)
{
   ...
   */
  if (BOARD(m, n) == EMPTY_T) {
    for (i = -1; i <= 1; i++)
      for (j = -1; j <= 1; j++)
	if (BOARD(m+i, n+j) == EMPTY_T)
  ...
  for (i = 0; i < board_size; i++)
	...
	if (IS_STONE(BOARD(i+di, j+dj))
	  ...
	  if (k < 4) {
	    ...
	    if (BOARD(i+di, j) == EMPTY_T || countlib(POS(i+di, j)) <= 2
		|| BOARD(i, j+dj) == EMPTY_T || countlib(POS(i, j+dj)) <= 2)
	      values[POS(i, j)] += contribution;
	    break;
	  }
	}
				  ...
				  char connection_shadow[BOARDMAX])
{
  store_persistent_cache(&connection_cache, routine, str1, str2, NO_MOVE, NULL,
			 ...
			 tactical_nodes, connection_shadow, EMPTY_T);
}
			       ...
			       char breakin_shadow[BOARDMAX])
{
  store_persistent_cache(&breakin_cache, routine,
			 ...
			 tactical_nodes, breakin_shadow, EMPTY_T);
}
			   ...
			   float contribution, char active_board[BOARDMAX])
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (k < 4) {
	  ...
	  if (board[pos3] == EMPTY_T || countlib(pos3) <= 2
	      || board[pos4] == EMPTY_T || countlib(pos4) <= 2)
	    values[pos] += 0.5 * contribution;
	  break;
	}
      }
     ...
     */
    if (k == 8 && active_board[pos] == EMPTY_T) {
      values[pos] -= 0.5 * contribution;
    }
  }
}
